...
if command -v java >/dev/null 2>&1; then
  jv=$(java -version 2>&1 | head -1 | sed 's/.*version "\([^"]*\)".*/\1/')
  major=${jv%%.*}
  if [ "${major:-0}" -ge 21 ] 2>/dev/null; then ok "java $jv"
  else bad "java $jv -- the exercises target Java 21, install a JDK 21 or later"; fi
else bad "java not found -- install a JDK 21 or later"; fi

if command -v mvn >/dev/null 2>&1
then ok "maven $(mvn -v 2>/dev/null | head -1 | awk '{print $3}')"
else bad "mvn not found -- install Apache Maven"; fi
...
if [ -n "$busy" ]; then
  ...
  printf '  (SimpleMessaging/src/main/java/simplemessaging/Channel.java and SimpleEventing/src/main/java/simpleeventing/Stream.java). See README.md.\n'
fi
...
# This is the other slow part to do at home: the first build downloads every dependency the three
# exercises use into ~/.m2, and a venue network is a poor place to discover that.
for d in ../01-message-pump ../02-failing-well ../03-streams; do
  if (cd "$d" && mvn -q package >/dev/null 2>&1)
  then ok "$(basename "$d") builds"
  else bad "$(basename "$d") does not build -- run 'mvn package' in it to see why"; fi
done

# The take-home, reported but not counted -- see 'note' at the top of this file.
if (cd ../04-lookup && mvn -q package >/dev/null 2>&1)
then ok "04-lookup builds (optional, take home)"; ex4="builds"
     ...
     ex4="DOES NOT BUILD -- run 'mvn package' in 04-lookup to see why"; fi
