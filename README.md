...
# Practical Messaging — Java exercises #
...
```
git clone <this repo>
cd Practical-Messaging-Java/00-setup
./prereqs.sh
```

`prereqs.sh` builds all three exercises, which is also what pulls every dependency into `~/.m2`.
Doing that at home is most of the point of running it at home.

**Every run command below is `java -jar .../target/<App>.jar`, so it runs the jar Maven built and
not your source.** After you change any code, rebuild that exercise before you run it:

```
mvn -q package          # from the exercise directory, e.g. 01-message-pump/
```

You need **Docker** and **a JDK 21 or later, plus Maven**. That is all. No cloud account, no API key, and
...
> `localhost:` constants in `SimpleMessaging/src/main/java/simplemessaging/Channel.java` and `SimpleEventing/src/main/java/simpleeventing/Stream.java`.
