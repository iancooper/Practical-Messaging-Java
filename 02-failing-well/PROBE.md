...
- `PlaceOrderHandler.handle` takes a `PlaceOrder`. `Model/pom.xml` has no broker reference
...
```
cd 02-failing-well
java -jar Receiver/target/Receiver.jar  # terminal 1
java -jar Sender/target/Sender.jar      # terminal 2
```
...
|---|---|
| `SimpleMessaging/src/main/java/simplemessaging/Channel.java` | the topology, drawn in the comment. Read the comment |
| `SimpleMessaging/src/main/java/simplemessaging/DataTypeChannelConsumer.java` | the five things the pump can do with a message |
| `SimpleMessaging/src/main/java/simplemessaging/MessagePump.java` | the `catch` block. All of it |
...
```
java -jar Receiver/target/Receiver.jar | tee /tmp/probe-a.log  # terminal 1
java -jar Sender/target/Sender.jar unmappable                  # terminal 2
```
...
```
java -jar Sender/target/Sender.jar poison  # NOPE-404: never in the catalogue, ever
```
...
```
java -jar Sender/target/Sender.jar flaky
```
...
**One file: `SimpleMessaging/src/main/java/simplemessaging/MessagePump.java`.** The topology is already declared and correct;
...
- `consumer.retriesSoFar(delivery)` tells you how many times this message has already been round the
...
And with all of that going on, `java -jar Sender/target/Sender.jar` must still place a good order
