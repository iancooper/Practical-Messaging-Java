...
```
cd 03-streams
java -jar Receiver/target/Receiver.jar              # RabbitMQ consumer AND Kafka producer
java -jar StreamConsumer/target/StreamConsumer.jar  # Kafka consumer, counts what it sees
java -jar Sender/target/Sender.jar                  # one order
```
...
| file | |
|---|---|
| `Model/src/main/java/model/PlaceOrderHandler.java` | the handler. **Nothing in it is wrong** |
| `SimpleEventing/src/main/java/simpleeventing/EventStreamConsumer.java` | the stream's version of the pump. Read the comment at the top |
| `SimpleEventing/src/main/java/simpleeventing/Stream.java` | the topology — and compare its length with `SimpleMessaging/src/main/java/simplemessaging/Channel.java` |

1. **`Channel.java` declares four queues. `Stream.java` declares one topic.** Exercise 2 used the
...
**Now do it the other way round.** Move the acknowledgement in `SimpleMessaging/src/main/java/simplemessaging/MessagePump.java`
so it happens *before* `handler.handle(...)` instead of after. Reset, and run the probe again — but
...
```
java -jar Sender/target/Sender.jar slow  # then kill the receiver during the lookup, before the event
```
...
```
DUAL_WRITE_WINDOW=15 java -jar Receiver/target/Receiver.jar  # terminal 1
java -jar Sender/target/Sender.jar                           # terminal 2, then look at ../00-setup/queues.sh
```
...
**Put the acknowledgement back after `handler.handle(...)` before you go on.** Probes B and C do
...
```
../00-setup/reset.sh
java -jar StreamConsumer/target/StreamConsumer.jar  # terminal 1, leave it running
java -jar Sender/target/Sender.jar bad-event        # appends an unreadable record to the topic
```
...
```
java -jar Receiver/target/Receiver.jar      # terminal 2
java -jar Sender/target/Sender.jar burst 6  # terminal 3
../00-setup/lag.sh
```
...
```
../00-setup/reset.sh
java -jar Receiver/target/Receiver.jar      # terminal 1, then send a few good orders and stop it
java -jar Sender/target/Sender.jar burst 4  # terminal 2
```

Now change `Stream.CONSUMER_GROUP` in `SimpleEventing/src/main/java/simpleeventing/Stream.java` to any new name and run the stream
consumer again.
