...
java -jar StreamConsumer/target/StreamConsumer.jar  # Kafka consumer, counts duplicates
java -jar Sender/target/Sender.jar                  # one order
java -jar Sender/target/Sender.jar bad-event        # an unreadable record, straight onto the stream

DUAL_WRITE_WINDOW=15 java -jar Receiver/target/Receiver.jar  # widens the gap so you can aim at it
```
...
| `Model/src/main/java/model/PlaceOrderHandler.java` | places the order, then publishes the event. Also correct |
