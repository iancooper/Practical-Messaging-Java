...
`Model/src/main/java/model/Catalogue.java` is a `Map<String, BigDecimal>`. It has been standing in for reference
...
specified: `org.xerial:sqlite-jdbc` is one dependency, it brings its own native library with it so
there is nothing to install on the machine, and the database is a file you can delete. No
container, no port, nothing else to go wrong on a laptop.
...
**And it is a check rather than a matter of opinion.** `Model/pom.xml` declares no
JDBC driver and `LocalCopy/` is the only module that does, so the domain cannot name
a type it has no dependency on and Maven will tell you so. Add `org.xerial:sqlite-jdbc` to the
domain's POM to make that error go away and you have made the mistake.

### The `PlaceOrderHandler` must not change either ###

If it does, the lookup has leaked into your domain the way `GetResponse` did in exercise 1 —
...
```
java -jar PriceConsumer/target/PriceConsumer.jar   # terminal 1, leave it running
java -jar Receiver/target/Receiver.jar             # terminal 2, leave it running
java -jar PriceSeeder/target/PriceSeeder.jar seed  # terminal 3: publish a starting price for every SKU
java -jar Sender/target/Sender.jar                 # terminal 3: place an order
```
...
```
java -jar PriceSeeder/target/PriceSeeder.jar set WIDGET-1 11.99  # terminal 3, watch terminal 1 print the gap
java -jar Sender/target/Sender.jar                               # then place an order and see which price it got
```
...
java -jar PriceSeeder/target/PriceSeeder.jar set WIDGET-1 49.99
java -jar Sender/target/Sender.jar
```
...
```
java -jar Sender/target/Sender.jar         # WIDGET-1: fine, but we have no copy yet
# ...now start the price consumer and seed it, then...
java -jar Sender/target/Sender.jar poison  # NOPE-404: not a thing, and never will be
```
...
```
PRICE_WRITE_WINDOW=15 java -jar PriceConsumer/target/PriceConsumer.jar  # terminal 1
java -jar PriceSeeder/target/PriceSeeder.jar set WIDGET-1 77.77         # terminal 3
kill -9 <the price consumer's PID>                                      # while the window is open
java -jar PriceConsumer/target/PriceConsumer.jar                        # start it again and watch
```
...
**Now reverse the two lines in `PriceConsumer/src/main/java/priceconsumer/PriceConsumer.java`** — commit the offset first, then apply —
and run it again.
