...
```
java -jar Receiver/target/Receiver.jar  # the pump
java -jar Sender/target/Sender.jar      # sends one order
```
...
| `SimpleMessaging/` | the messaging gateway. Given to you, and correct — **except `MessagePump.java`**, which lives here and is the exercise |
| `Model/` | the domain — an order, a catalogue, and a handler |
| `Sender/`, `Receiver/` | two console apps |

**The pump in `SimpleMessaging/src/main/java/simplemessaging/MessagePump.java` is deliberately wrong.** It runs. Do not copy it.
