...
### 1. The handler took a `GetResponse` ###
...
- **The `Model` project had to reference `com.rabbitmq:amqp-client`.** The domain now has a compile-time
...
- **The handler cannot be called by anything else.** Not a unit test, not an HTTP endpoint, not
  ...
  `GetResponse`, which means fabricating a delivery tag and a byte array.
- **The pump cannot tell your failure from its own.** A `JsonProcessingException` and an
...
calls it. The handler takes `PlaceOrder` and returns. Delete the `dependency` from
`Model/pom.xml` and let the compiler find anything you missed.
...
- **It must fail in its own way.** When the body will not deserialize, the mapper catches
  ...
  *I read it and the work failed*, and exercise 2's entire fix is that distinction. A mapper
  that lets `JsonProcessingException` escape has done the translation and not the classification.
...
```
    ...
    consumer.acknowledge(delivery.getEnvelope().getDeliveryTag());   // <-- here
