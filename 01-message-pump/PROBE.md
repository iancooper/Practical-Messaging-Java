...
```
cd 01-message-pump
java -jar Receiver/target/Receiver.jar  # terminal 1: the pump
java -jar Sender/target/Sender.jar      # terminal 2: sends one order
```
...
| file | |
|---|---|
| `SimpleMessaging/src/main/java/simplemessaging/MessagePump.java` | the pump: **Get → Translate → Dispatch → Handle** |
| `Model/src/main/java/model/PlaceOrderHandler.java` | your application code |
| `Model/pom.xml` | the domain project's dependencies |

1. **Find the four stages of the pump in `MessagePump.java`.** Get is labelled. Where are the
   other three?
2. **`PlaceOrderHandler.handle` takes a `GetResponse`.** What is a `GetResponse`, which
   package does it come from, and what is it doing in a method about placing orders?
3. **`Model/pom.xml` has a `dependency` to `com.rabbitmq:amqp-client`.** `Model` is the domain.
...
```
java -jar Sender/target/Sender.jar unmappable
```
...
```
java -jar Sender/target/Sender.jar poison
```
...
```
java -jar Sender/target/Sender.jar  # a perfectly good WIDGET-1
```
...
```
java -jar Sender/target/Sender.jar slow
```
...
`handle(GetResponse)` becomes `handle(PlaceOrder)`. When you are done:

- `Model/pom.xml` **has no reference to `com.rabbitmq:amqp-client`.** Delete it and make the build pass.
  That is your check, and it is a build error rather than a matter of opinion.
- Nothing in `Model/` has a `import com.rabbitmq.client`.
...
**And when all three are done, delete the banner at the top of `SimpleMessaging/src/main/java/simplemessaging/MessagePump.java`.** It says the
...
- Send `java -jar Sender/target/Sender.jar burst 50` and watch the queue drain. Then start a
